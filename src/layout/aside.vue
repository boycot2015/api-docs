        ...
        <template v-for="item in routes" :key="item.path">
            <el-sub-menu :index="item.path" v-if="item.children && item.children.length && !item.meta.hideChildren">
                <template #title>
                    ...
                    <span :title="item.meta.title" v-if="item.meta.title">{{item.meta.title.slice(0, 6)}}</span>
                </template>
                <el-menu-item  v-for="child in item.children" :key="child.path" :index="child.path">
                    <el-icon v-if="child.meta && child.meta.icon" ><IconifyIcon :name="child.meta.icon" /></el-icon>
                    <template #title v-if="child.meta && child.meta.title"><span :title="child.meta.title">{{child.meta.title.slice(0, 10)}}</span></template>
                </el-menu-item>
            </el-sub-menu>
            <el-menu-item :index="item.path" v-else-if="!item.meta.hideInMenu">
                ...
                <template #title v-if="item.meta && item.meta.title"><span :title="item.meta.title">{{item.meta.title.slice(0, 6)}}</span></template>
            </el-menu-item>
        </template>
    </el-menu>
